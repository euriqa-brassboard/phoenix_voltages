...
function get_electrodes(xpos_um)
    if abs(xpos_um - center_um) <= 100
        ...
        eles = Mappings.find_electrodes(solution.electrode_index,
                                        xpos_um, min_num=16, min_dist=450)
        union!(eles, center_electrodes)
    end
    return sort!(collect(eles))
end

const move_x2_init = 6.0
...
function get_barriers(xpos_um)
    ...
    add_barrier_pairs!(barriers, xpos_um, 245, 4.5)
    ...
    if xpos_um <= loading_um + 30
        # Add a loading barrier for boil off
        push!(barriers, (xpos_um - 170, xpos_um, 2.0))
        push!(barriers, (xpos_um + 350, xpos_um, 10.0))
    end
    return barriers
end
...
function create_frame1(eles, xpos_um, load_dz=0)
    ...
    if load_dz != 0
        push!(freedom, ((dz_block(), move_zero), load_dz * 0.8, load_dz * 1.5))
    end
    if shape_relax > 0
        if xpos_um < -70
            push!(freedom, ((dx_block(), move_zero),
                            -10 * shape_relax, 10 * shape_relax))
        else
            push!(freedom, ((dx_block(), move_zero),
                            -500 * shape_relax, 500 * shape_relax))
        end
    ...
    if xpos_um < -70
        push!(freedom, ((center_zero, dx_block()),
                        -200 * shape_relax, 200 * shape_relax))
    else
        push!(freedom, ((center_zero, dx_block()),
...
function create_frame(xpos_um, load_dz=0)
    @show xpos_um
    eles = get_electrodes(xpos_um)
    if xpos_um == 0
        frame = create_frame0(eles, xpos_um)
    else
        frame = create_frame1(eles, xpos_um, load_dz)
    end
    add_barrier!(frame, xpos_um)
    return frame
end

const prefix = joinpath(@__DIR__, "../data/merge_coeff_20240815")
...
const xpos_ums = loading_um:5:center_um

const results = Dict{String,Any}[]
for xpos_um in xpos_ums
    @time begin
        load_dz_r = get_ratio(xpos_um, loading_um, loading_um + 30)
        load_dz = min(load_dz_r, 1 - load_dz_r) * 80000.0
        frame = create_frame(xpos_um, load_dz)
