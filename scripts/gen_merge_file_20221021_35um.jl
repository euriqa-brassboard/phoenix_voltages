...
# These are not real positions, in fact it's the opposite of the parking spot position.
# These are just the number we use to mark spot in the merging solution
# so that we can abuse the basic shuttle experiment to do what we need during testing.
const xpos_ums_eff2 = 106:455
const loading_pos_um = -3045
@assert length(transfer.transfer_solutions[1]) == length(xpos_ums1)
@assert length(transfer.transfer_solutions[2]) == length(xpos_ums_eff2)
...
""")
    ...
    merge_end_name = pos_to_name(xpos_ums_eff2[end])
    move_end_name = "555um"

    prev_pos = xpos_ums1[end]
    ...
    println(xml_io, "    <ShuttleEdge idleCount=\"500\" steps=\"3\" startLength=\"0\" stopLength=\"0\" startLine=\"$(prev_lineidx)\" startName=\"$(pos_to_name(prev_pos))\" stopLine=\"$(section2_end)\" stopName=\"$(merge_end_name)\"/>")
